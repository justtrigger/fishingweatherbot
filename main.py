...
class ForecastMessage:
    def __init__(self,data, message, saved_user_data) -> None:
        ...
        self.length_of_the_day = (dt.datetime.fromtimestamp(data["sys"]["sunset"]) -
    ...
    def __str__(self) -> str:
        return (f"{saved_user_data[self.message.from_id]['forecast_type']}: "
                ...
                f"Влажность: {self.humidity}%\nДавление: {round(self.pressure/1.333)} мм.рт.ст\n"
                f"Ветер: {self.wind_dir},\n{self.wind_speed} м/с, порывы до {self.wind_gust} м/с\n"
